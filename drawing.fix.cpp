...
int main() {
	...
	*/
	...
	// menu things, whenever we do put something on that page
	...
	vector<bool> isSpiderDead;
	for (int i = 0; i < spiderNum; i++) {
		isSpiderDead.push_back(false);
	}
	...
	vector<bool> isDogDead;
	for (int i = 0; i < dogNum; i++) {
		isDogDead.push_back(false);
	}
	...
	vector<bool> areBatsDead;
	for (int i = 0; i < batsNum; i++) {
		areBatsDead.push_back(false);
	}
	...
	vector<bool> isBearDead;
	for (int i = 0; i < bearNum; i++) {
		isBearDead.push_back(false);
	}
	...
	vector<bool> isHumanDead;
	for (int i = 0; i < humanNum; i++) {
		isHumanDead.push_back(false);
	}
	...
	while (window.isOpen()) {
		...
		while (it != mutants.end()) {
			...
			if (m->isDead() == true) { //remove dead monsters
				it = mutants.erase(it);
				delete m;
				isMutantDead[getIndex(mutants, m)] = true;
				cout << "mutant died\n";
				continue;
			}
		...
		if (screen >= 4 && screen < 9) { // at this point it's all monster rooms from here for now
			...
			if (screen == 4) {
				for (int i = 0; i < spiderNum; i++) {
					if (isSpiderDead[i] == false) {
						window.draw(spiders[i]->getSprite())); //if the mutant isn't dead draw it
					}
				}
			}
			if (screen == 5) {
				for (int i = 0; i < dogNum; i++) {
					if (isDogDead[i] == false) {
						window.draw(dogs[i]->getSprite())); //if the mutant isn't dead draw it
					}
				}
			}
			if (screen == 6) {
				for (int i = 0; i < batsNum; i++) {
					if (areBatsDead[i] == false) {
						window.draw(bats[i]->getSprite())); //if the mutant isn't dead draw it
					}
				}
			}
			if (screen == 7) {
				for (int i = 0; i < bearNum; i++) {
					if (isBearDead[i] == false) {
						window.draw(bears[i]->getSprite())); //if the mutant isn't dead draw it
					}
				}
			}
			if (screen == 8) {
				for (int i = 0; i < humanNum; i++) {
					if (isHumanDead[i] == false) {
						window.draw(humans[i]->getSprite())); //if the mutant isn't dead draw it
					}
				}
			}
		}
