...
int main() {
	...
	while (window.isOpen()) {
		...
		for (int i = 0; i < monsterNum; i++) {
			...
			if (screen == 4 && i < spiderNum) {
				if ((directionsx[mDir] < 0 && !mutantBoundingBoxes[i].intersects(leftLineBoundingBox)) ||
					(directionsx > 0 && !mutantBoundingBoxes[i].intersects(caveWallBoundingBox)) ||
				...
				cout << "X: " << m->getSprite()->getPosition().x << "Y: " << m->getSprite()->getPosition().y << endl;
			}
		}
