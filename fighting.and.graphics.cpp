...
int main() {
	...
	*/
	...
	while (window.isOpen()) {
		...
		playerBounds = player.getGlobalBounds(); //Checks players bounds to update them as the player moves
		...
		if (screen >= 4 && screen < 9) { // at this point it's all monster rooms from here for now
			...
			if (isItemInInventory[index] == false) {
...
Sprite createItem(Texture *texture, int windowX) {
	...
	while(x >= (windowX - 200)) { // If item spawns out of bounds (i.e. in the wall) it resets the x value
		x = rand() % (windowX / 4) * 4;
		sprite.setPosition(x, y);
	}
